    ...
    for(unsigned int ts = 1; ts<seq[0].size(); ts++)
    {
        ...
        for(unsigned int i = 0; i<a_col; i++)
        {
            for(unsigned int j = 0; j<a_col; j++)
                delta_temp[j] = delta[j][ts-1]*a[j][i]*b[i][observpos];
        }

        // Find the max element in delta_temp
        for(unsigned int j = 0; j<delta_temp.size(); j++)
        {
            max_pos = max_element(delta_temp.begin(), delta_temp.end()) - delta_temp.begin();
            delta_idx[j][ts] = max_pos;

            delta[j][ts] = *max_element(delta_temp.begin(), delta_temp.end());
        }
    }
